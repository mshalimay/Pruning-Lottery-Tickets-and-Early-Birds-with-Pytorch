...
__all__ = ['resnetb', 'resnet']

"""
preactivation resnet with bottleneck design.
"""
# TODO: correct basicblock implementation
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, cfg[0], kernel_size=3, stride=stride,
                     padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(cfg[0])
        self.conv2 = nn.Conv2d(cfg[0], planes * self.expansion, kernel_size=3, stride=1,
                     padding=1, bias=False)
...
class Resnet(nn.Module):
    def __init__(self, depth=164, dataset='cifar10', use_bb=False, cfg=None):
        super(Resnet, self).__init__()
        assert (depth - 2) % 9 == 0, 'depth should be 9n+2'

        n = (depth - 2) // 9
        # block = Bottleneck
        block = BasicBlock if use_bb else Bottleneck

        if cfg is None:
            if use_bb:
                cfg = [[16] * n * 3, [32] * n * 3, [64] * n *3, [64]]
            else:                
...
def resnetb(**kwargs):
    model = Resnet(use_bb=False, **kwargs)
    # write neural network to file
    return model

def resnet(**kwargs):
    model = Resnet(use_bb=True, **kwargs)
    # write neural network to file
    return model
